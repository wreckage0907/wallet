# Specs

Working notes that do not belong in the README. The README describes what Wallet is and
how to run it; this directory holds the things to pick up later — open findings, the
boundaries the design relies on, and the development workflow.

| File | What it holds |
|---|---|
| [`audit-2026-08-30.md`](audit-2026-08-30.md) | Findings from the first full audit, most severe first |
| [`security-boundaries.md`](security-boundaries.md) | What actually isolates one user's money from another's, and where those boundaries stop |
| [`development.md`](development.md) | Building the frontend, running the test suites, CI, repo conventions |
| [`testing.md`](testing.md) | Where a test file goes, and how the Python suites are written |
